...
namespace combi
{
	...
	{
		this->n = n;				///кол эл-тов исх массива
		this->sset = new short[n];	///массив индексов тек подмн-ва 
		this->reset();
	};

	void  subset::reset()	///сбросить генератор 
	{
		this->sn = 0;		///кол-во эл-тов тек подмн-ва
	...
	short subset::getfirst()		///сформ массив индексов по бит маске
	  {
		...
		{
			if (buf & 0x1)
				this->sset[this->sn++] = i;///массив индексов тек подмн-ва  
			buf >>= 1;                                 
		}
		return this->sn;///кол-во эл-тов тек подмн-ва   
	  };
